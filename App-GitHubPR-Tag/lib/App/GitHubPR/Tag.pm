...
# Options
has 'mergeable' => (is => 'rw', isa => 'Bool', predicate => 'has_mergeable');
has 'repo-user' => (is => 'rw', isa => 'Str', required => 1, accessor => 'repo_user');
has 'repo-name' => (is => 'rw', isa => 'Str', required => 1, accessor => 'repo_name');
...
sub init_pull_request {
    my $self = shift;
    $self->pull_request->set_default_user_repo(
        $self->repo_user,
        $self->repo_name
    );
}
